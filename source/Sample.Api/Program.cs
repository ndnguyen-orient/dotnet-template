...
// OpenTelemetry: traces, metrics, and logs for ASP.NET Core, HttpClient, the runtime, EF Core,
// and Npgsql, exported via OTLP. The exporter honors the standard OTEL_EXPORTER_OTLP_* env vars
// (endpoint defaults to http://localhost:4317); point it at your collector in each environment.
// All three signals share one resource, so a backend (e.g. the Aspire dashboard) correlates a
// trace with its own logs.
builder.Services
    .AddOpenTelemetry()
